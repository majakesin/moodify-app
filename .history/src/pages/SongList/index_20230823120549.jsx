...
const SongList = () => {
    ...
    const form = useMemo(() => {
        ...
        return (
            <form onSubmit={onSubmit} className={styles.form} >
                <label> <div className={styles.text}> Name </div>
                    <input type="text" name="name" value={name} onChange={onChange} />
                </label>
                <label> <div className={styles.text}> Author </div>
    ...
    //Example of the memoization, also for key in the list we need to use something like unique
    const songsList = useMemo(() => (
        songsToDisplay.map((song, index) => (
            <Song key={song.name} {...song} index={index} onDelete={onDelete} />
        ))), [songsToDisplay, onDelete]);

    return (
        <div>
            {songsList}
            <button onClick={onShowForm}><IoIosAddCircle /></button>
            {form}
        </div>
    )
}
